...
import type { Category, ChatThread, Favorite, Language, Listing, ListingCondition, Location, Message, Profile, User } from "@lwaye/shared";
...
async function apiGet<T>(path: string, sessionToken?: string): Promise<T> {
  const response = await fetch(`${API_BASE_URL}${path}`, { headers: sessionToken ? { Authorization: `Bearer ${sessionToken}` } : undefined });
  if (!response.ok) {
...
async function apiPost<T>(path: string, body?: unknown, sessionToken?: string): Promise<T> {
  const response = await fetch(`${API_BASE_URL}${path}`, { method: "POST", headers: { "Content-Type": "application/json", ...(sessionToken ? { Authorization: `Bearer ${sessionToken}` } : {}) }, body: body ? JSON.stringify(body) : undefined });
  if (!response.ok) {
    const payload = (await response.json().catch(() => undefined)) as { message?: string } | undefined;
    throw new Error(payload?.message ?? `Request failed: ${response.status}`);
  }
...
export default function App() {
  ...
  const seller = useMemo(() => bootstrap?.users.find((user) => user.profileType === "small_business") ?? bootstrap?.users[0] ?? null, [bootstrap]);
  const viewerProfile = useMemo(() => viewer ? bootstrap?.profiles.find((profile) => profile.userId === viewer.id) ?? null : null, [bootstrap, viewer]);
  const selectedThread = useMemo(() => !selectedThreadId ? threads[0] ?? null : threads.find((thread) => thread.id === selectedThreadId) ?? null, [selectedThreadId, threads]);
  const selectedListing = useMemo(() => !selectedThread ? null : feed.find((listing) => listing.id === selectedThread.listingId) ?? null, [feed, selectedThread]);
  ...
  useEffect(() => { if (!bootstrap) return; setSelectedCategoryId((current) => current ?? bootstrap.categories[0]?.id ?? null); setSelectedLocationId((current) => current ?? bootstrap.locations[0]?.id ?? null); }, [bootstrap]);

  async function hydrate(activeSessionToken: string) {
    setIsLoading(true); setErrorMessage(null);
    try {
      const [bootstrapPayload, listingsPayload, mePayload] = await Promise.all([apiGet<BootstrapPayload>("/v1/bootstrap", activeSessionToken), apiGet<ListingsPayload>("/v1/listings", activeSessionToken), apiGet<{ user: User }>("/v1/auth/me", activeSessionToken)]);
      setBootstrap(bootstrapPayload); setFeed(listingsPayload.items); setViewer(mePayload.user); setLanguage(mePayload.user.preferredLanguage);
    } catch (error) {
  ...
  async function loadFeed(nextSearch?: string) {
    ...
    try {
      ...
      setFeed(payload.items); setErrorMessage(null);
    } catch (error) { setErrorMessage((error as Error).message); } finally { setIsRefreshingFeed(false); }
  }

  async function loadUserData(userId: string) {
    try {
      const [threadsPayload, favoritesPayload] = await Promise.all([apiGet<ChatThread[]>(`/v1/threads/${userId}`, sessionToken ?? undefined), apiGet<Favorite[]>(`/v1/favorites/${userId}`, sessionToken ?? undefined)]);
  ...
  async function handleStartOtp() {
    ...
    try {
      ...
      setDemoCodeHint(payload.code); setOtpCode(payload.code); Alert.alert("Verification code sent", `Demo code: ${payload.code}`);
  ...
  async function handleLogout() {
    ...
    setSessionToken(null); setViewer(null); setBootstrap(null); setFeed([]); setThreads([]); setMessages([]); setFavorites([]); setSelectedThreadId(null); setDemoCodeHint(null); setSearch("");
  }
  ...
  async function handleCreateListing() {
    ...
    try {
      await apiPost<Listing>("/v1/listings", { sellerId: viewer.id, title: listingTitle.trim(), description: listingDescription.trim(), priceETB: priceValue, negotiable: listingNegotiable, categoryId: selectedCategoryId, condition: listingCondition, locationId: selectedLocationId, photoUrls: listingPhotoUrl.trim() ? [listingPhotoUrl.trim()] : [] }, sessionToken ?? undefined);
      setListingTitle(""); setListingDescription(""); setListingPrice(""); setListingNegotiable(true); setListingCondition(DEFAULT_CONDITION); setListingPhotoUrl(""); setTab("browse"); await loadFeed(); Alert.alert("Listing created", "Your listing is now live in the feed.");
