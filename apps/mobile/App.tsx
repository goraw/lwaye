...
async function apiUploadImage(uri: string, sessionToken: string, fileName?: string, mimeType?: string): Promise<UploadPayload> {
  ...
  formData.append("image", { uri, name: fileName ?? `listing-${Date.now()}.jpg`, type: mimeType ?? "image/jpeg" } as never);
  const response = await fetch(`${API_BASE_URL}/v1/uploads/image`, { method: "POST", headers: { Authorization: `Bearer ${sessionToken}` }, body: formData });
  if (!response.ok) throw new Error(await readError(response));
...
export default function App() {
  ...
  async function handleLogout() {
    ...
    setSessionToken(null); setViewer(null); setBootstrap(null); setFeed([]); setThreads([]); setMessages([]); setFavorites([]); setSelectedThreadId(null); setDemoCodeHint(null); setSearch(""); setOtpCode(""); setListingPhotoUrl("");
  }
